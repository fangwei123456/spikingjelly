...
@pytest.mark.parametrize(
    ("kind", "node_kwargs"),
    [
        ("if", {"v_reset": 0.0, "detach_reset": False}),
        ("if", {"v_reset": None, "detach_reset": True}),
        (
            "lif",
            {
                "tau": 2.0,
...
def test_single_step_triton_matches_torch_training(kind, node_kwargs):
    ...
    torch_node = node_cls(step_mode="s", backend="torch", **node_kwargs).cuda()
    triton_node = node_cls(step_mode="s", backend="triton", **node_kwargs).cuda()
    assert "triton" in triton_node.supported_backends
    ...
    _assert_close(y_triton, y_torch, torch.float32)
    _assert_close(triton_node.v, torch_node.v, torch.float32)

    loss_torch = (y_torch * weight).sum() + 0.125 * torch_node.v.sum()
    loss_triton = (y_triton * weight).sum() + 0.125 * triton_node.v.sum()
    loss_torch.backward()
    loss_triton.backward()

    _assert_close(x_triton.grad, x_torch.grad, torch.float32)
    _assert_close(v_triton.grad, v_torch.grad, torch.float32)
    if kind == "plif":
        _assert_close(triton_node.w.grad, torch_node.w.grad, torch.float32)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
@pytest.mark.parametrize("kind", ["if", "lif", "plif"])
def test_single_step_functional_triton_interface(kind):
    pytest.importorskip("triton")
    x = torch.randn(7, 13, device="cuda")
    ...
    if kind == "if":
        torch_node = neuron.IFNode(
            v_reset=None,
            surrogate_function=surrogate_function,
        ).cuda()
        triton_function = functional.if_single_step_triton
        triton_args = (
            x,
            v,
        ...
        torch_node = neuron.LIFNode(
            tau=3.0,
            decay_input=False,
            v_reset=None,
            surrogate_function=surrogate_function,
        ).cuda()
        triton_function = functional.lif_single_step_triton
        triton_args = (
            x,
            v,
            torch_node.tau,
            torch_node.decay_input,
            torch_node.v_threshold,
            torch_node.v_reset,
            surrogate_function,
        )
        ...
        triton_function = functional.plif_single_step_triton
        triton_args = (
            x,
            v,
            torch_node.w,
    ...
    with torch.inference_mode():
        ...
        actual_spike, actual_v = triton_function(*triton_args)
    _assert_close(actual_spike, expected_spike, torch.float32)
    _assert_close(actual_v, torch_node.v, torch.float32)
