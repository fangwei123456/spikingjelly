...
def _readATIS_tddat(
    ...
    if verbose:
        logger.info("Reading _td dat file... (%s)", file_name)
    ...
    if header:
        ...
        if ev_size != 8:
            logger.info("Wrong event size. Aborting.")
            return -1, -1, -1, -1
    else:  # set default ev type and size
        if verbose:
            logger.info("> No header. Setting default event type and size.")
        ev_size = 8
        ev_type = 0
    ...
    debug_progress = verbose and logger.isEnabledFor(logging.DEBUG)
    if verbose:
        logger.info("> The file contains %s events.", Nevents)
    ...
    file.close()
    if verbose:
        logger.info("> After loading events, actually found %s events.", ActualEvents)
    ...
    for ts in timestamps:
        if ts < 0:
            logger.info("Found a negative timestamp.")
    ...
    if drop_negative_dt:
        if verbose:
            logger.info("> Looking for negative dts...")
        ...
        while just_dropped:
            ...
            if len(index_neg) > 0:
                ...
                if verbose:
                    logger.info("> Removed %s events in %s passes.", drop_sum, nPasses)
