...
def _readATIS_tddat(
    ...
    logger.debug("Reading _td dat file... ({})", file_name)
    ...
    if header:
        ...
        logger.debug(
            "> Header exists. Event type is {}, event size is {}",
            ev_type,
            ev_size,
        )
        if ev_size != 8:
            logger.error(
                "Invalid event size in {}: expected 8, got {}; aborting.",
                file_name,
                ev_size,
            )
            file.close()
    ...
    logger.debug("> The file contains {} events.", Nevents)
    ...
    for i in np.arange(0, int(Nevents)):
        ...
        if i > events_restriction[1]:
            break
    file.close()
    logger.debug("> After loading events, actually found {} events.", ActualEvents)
    ...
    if np.any(timestamps < 0):
        logger.warning("Found negative timestamps in {}.", file_name)
    ...
    if drop_negative_dt:
        ...
        while just_dropped:
            ...
            if len(index_neg) > 0:
                ...
                logger.warning("> Removed {} events in {} passes.", drop_sum, nPasses)
        removed_events = drop_sum
    else:
        removed_events = -1
    logger.debug(
        "> Sequence duration: {:.2f}s, ts[0] = {}, ts[{}] = {}.",
