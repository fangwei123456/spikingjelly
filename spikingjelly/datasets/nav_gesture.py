...
def _readATIS_tddat(
    ...
    logger.debug("Reading _td dat file... (%s)", file_name)
    ...
    if header:
        ...
        logger.debug(
            "> Header exists. Event type is %s, event size is %s",
            ev_type,
            ev_size,
        )
        if ev_size != 8:
            logger.error(
                "Invalid event size in %s: expected 8, got %s; aborting.",
                file_name,
                ev_size,
            )
            return -1, -1, -1, -1
    ...
    logger.debug("> The file contains %s events.", Nevents)
    ...
    for i in np.arange(0, int(Nevents)):
        ...
        if debug_progress and i % dNEvents == 0:
            logger.debug("> %s%% events processed", i / dNEvents)
        if i > events_restriction[1]:
            break
    file.close()
    logger.debug("> After loading events, actually found %s events.", ActualEvents)
    ...
    if np.any(timestamps < 0):
        logger.warning("Found negative timestamps in %s.", file_name)
    ...
    drop_sum = 0
    if drop_negative_dt:
        logger.debug("> Looking for negative dts...")
        ...
        while just_dropped:
            ...
            while ii < (timestamps.size - 1):
                ...
                if dt < 0:  # alors ts en ii+1 plus petit que ii
                    ...
                    ii += 1
                    just_dropped = True
                if debug_progress and ii % dNEvents == 0:
                    logger.debug(
                        "> %s%% events processed (pass %s)", ii / dNEvents, nPasses
                    )
                ii += 1
            if len(index_neg) > 0:
                ...
                logger.warning("> Removed %s events in %s passes.", drop_sum, nPasses)
        removed_events = drop_sum
    else:
        removed_events = -1
    logger.debug(
        "> Sequence duration: %.2fs, ts[0] = %s, ts[%s] = %s.",
...
def _read_aedat_save_to_np(bin_file: Union[str, Path], np_file: Union[str, Path]):
    t, xy, p, _ = _readATIS_tddat(bin_file)
    x = xy[:, 0]
    y = 239 - xy[:, 1]
