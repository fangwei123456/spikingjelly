    ...
    class MultiStepEIFNodePTT(torch.autograd.Function):
        ...
        def create_fptt_kernel(hard_reset: bool, dtype: str):
            ...
            if dtype == 'fp32':
                ...
                if (index < neuron_num)
                {
                    ...
                    {
                        const int t = index + mem_offset;
                        h_seq[t] = __hfma(__hfma(hexp(__hdiv(__hsub(v_v_seq[t], theta_rh), delta_T)), delta_T, __hadd(__hsub(x_seq[t], v_v_seq[t]), v_rest)), reciprocal_tau, v_v_seq[t]);
        ...
        def create_bptt_kernel(sg_cuda_code_fun, hard_reset: bool, detach_reset: bool, dtype: str):
            ...
            if dtype == 'fp32':
                ...
                code += code_grad_v_to_h
                code += r'''
                        grad_h = __hfma(__hfma(__hfma(hexp(__hmul(__hsub(v_v_seq[t + neuron_num], theta_rh), reciprocal_delta_T)), reciprocal_tau, one_sub_reciprocal_tau), grad_h, grad_v_seq[t]), grad_v_to_h, __hmul(grad_spike_seq[t], grad_s_to_h));                      
                        grad_x_seq[t] = __hmul(grad_h, reciprocal_tau);
                    }
                grad_v_last[index] = __hmul(__hfma(hexp(__hmul(__hsub(v_v_seq[index], theta_rh), reciprocal_delta_T)), reciprocal_tau, one_sub_reciprocal_tau), grad_x_seq[index]);
                }
                }
                '''
            else:
