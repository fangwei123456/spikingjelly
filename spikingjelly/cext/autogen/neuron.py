...
class NeuronFPTT:
    ...
    @staticmethod
    def impl_fptt(neuron_name, hard_reset, extra_cuda_kernel_args='', extra_cuda_kernel_half_args='', extra_codes=''):
        ...
        if hard_reset:
            ret += f'{neuron_name}_hard_reset_fptt_kernel_half<<<blocks, threads>>>(x_seq.data_ptr<at::Half>(), s_seq.data_ptr<at::Half>(), v_seq.data_ptr<at::Half>(), __float2half(v_th), __float2half(v_reset), neuron_num, size{utils.add_comma(extra_cuda_kernel_args)});\n'
        else:
    ...
    def impl_cuda_kernel(neuron_name, hard_reset, charge_codes, charge_codes_half, extra_cuda_kernel_args='', extra_cuda_kernel_half_args=''):
        if hard_reset:
            ...
            ret = f'__global__ void {neuron_name}_soft_reset_fptt_kernel(const float* __restrict__ x_seq, float* __restrict__ s_seq, float* __restrict__ v_seq, const float v_th, const int neuron_num, const int size{utils.add_comma(extra_cuda_kernel_half_args)})\n'
        ...
        if hard_reset:
            ret += f'__global__ void {neuron_name}_hard_reset_fptt_kernel_half(const at::Half* __restrict__ x_seq, at::Half* __restrict__ s_seq, at::Half* __restrict__ v_seq, const half v_th, const half v_reset, const int neuron_num, const int size{utils.add_comma(extra_cuda_kernel_args)})\n'
        else:
            ret += f'__global__ void {neuron_name}_soft_reset_fptt_kernel_half(const at::Half* __restrict__ x_seq, at::Half* __restrict__ s_seq, at::Half* __restrict__ v_seq, const half v_th, const int neuron_num, const int size{utils.add_comma(extra_cuda_kernel_args)})\n'
    ...
    def test_IFNode():
        ...
        test_IFNode = cpp_extension.load(name='test_IFNode',
        ...
        with torch.no_grad():
            ...
            s_seq, v_seq = test_IFNode.IFNode_hard_reset_fptt(x_seq, v, v_th, v_reset)
            ...
            v_th = 1.
            v_reset = 0.
            s_seq, v_seq = test_IFNode.IFNode_hard_reset_fptt(x_seq.half(), v.half(), v_th, v_reset)
