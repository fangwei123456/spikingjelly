...
class Neuronal_Cell(nn.Module):
    ...
    def forward(self, x_in):
        ...
        x_3 = (
            self.cell_architecture[3](x_in)
            + self.cell_architecture[4](x_1)
            + self.cell_architecture[5](x_2)
        )
...
class Neuronal_Cell_backward(nn.Module):
        ...
        for col in range(0, 3):
            for row in range(col + 1, 4):
                ...
                if op == 0:
                    ...
                    self.cell_architecture_back.append(
                        nn.Sequential(
                            neuron.LIFNode(
                                v_threshold=args.threshold,
                                v_reset=0.0,
    ...
    def forward(self, x_in):
        ...
        x_2 = self.cell_architecture[1](x_in + self.last_xin) + self.cell_architecture[
            2
        ](x_1 + self.last_x1)
        x_3 = (
            self.cell_architecture[3](x_in + self.last_xin)
            + self.cell_architecture[4](x_1 + self.last_x1)
            + self.cell_architecture[5](x_2 + self.last_x2)
        )

        self.last_xin = (
            self.cell_architecture_back[0](x_1 + self.last_x1)
            + self.cell_architecture_back[1](x_2 + self.last_x2)
            + self.cell_architecture_back[2](x_3)
        )
        self.last_x1 = self.cell_architecture_back[3](
            x_2 + self.last_x2
        ) + self.cell_architecture_back[4](x_3)
...
class SNASNet(nn.Module):
    def __init__(self, args, con_mat):
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(
                self.in_channel,
                self.first_out_channel * self.channel_ratio,
                kernel_size=3,
        ...
        if args.celltype == "forward":
            ...
            self.cell1 = Neuronal_Cell_backward(
                args,
                self.first_out_channel * self.channel_ratio,
                self.first_out_channel * self.channel_ratio,
                self.con_mat,
        ...
        self.resdownsample2 = nn.AvgPool2d(
            self.second_avgpooling, self.second_avgpooling
        )
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(
                256
                * self.channel_ratio
                * (self.img_size // self.spatial_decay)
                * (self.img_size // self.spatial_decay),
                self.classifier_inter_ch,
                bias=False,
            ),
            neuron.LIFNode(
                v_threshold=args.threshold,
