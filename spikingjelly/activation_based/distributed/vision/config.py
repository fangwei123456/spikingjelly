...
class ModelConfig:
    ...
    **中文：** 视觉 SNN 模型配置的基类。子类通过 ``builder`` 类变量声明可导入的
    builder，并至少定义时间步和类别数。builder 负责模型结构及其张量并行策略。

    :param time_steps: 每个图像样本的 SNN 时间步数。
    :type time_steps: int
    :param num_classes: 分类类别数。
    :type num_classes: int
    :raises ValueError: 参数不是正整数。
    ...
    **English:** Base configuration for vision SNNs. Subclasses declare an
    ...
    class counts. The builder owns model construction and tensor parallelism.

    :param time_steps: SNN time steps for each image sample.
    :type time_steps: int
    :param num_classes: Number of classification classes.
    :type num_classes: int
    :raises ValueError: If either value is not positive.
    """
...
class ModelBuilder(abc.ABC):
    ...
    **中文：** model config 与 architecture-specific 实现之间的 seam。实现只负责
    ...
    训练生命周期由 :func:`spikingjelly.activation_based.distributed.vision.train` 负责。
    ...
    **English:** Seam between model configuration and architecture-specific
    ...
    and return module paths suitable for native FSDP2. The training lifecycle belongs to
    :func:`spikingjelly.activation_based.distributed.vision.train`.
    """
...
**中文：** 配置图像数据、优化器、训练进度、DP/FSDP2、通道 TP、PP、精度、
SpikingJelly memopt 与 checkpoint。全局 batch 为 ``batch_size * DP``，不乘 TP
或时间步。dataset builder 接收 ``dataset_kwargs`` 并返回 train/validation Dataset。

**English:** Configure image data, optimization, progress, DP/FSDP2, channel TP, PP,
precision, SpikingJelly memopt, and checkpoints. Global batch size is
...
:param model: Architecture-specific model configuration. / 模型专项配置。
