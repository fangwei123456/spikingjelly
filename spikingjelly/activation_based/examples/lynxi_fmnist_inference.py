...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-pt-path",
        default="/home/cxhpc/fangwei/tempdir/fmnist_test/logs/T4_b128_sgd_lr0.1_c128_cupy/checkpoint_max.pth",
        type=str,
        help="checkpoint file path for conv_fashion_mnist.CSNN",
    )
    parser.add_argument(
        "-out-model-path",
        default="/home/cxhpc/fangwei/tempdir/fmnist_test/lynxi_model",
        type=str,
        help="path for saving the model compiled by lynxi",
    )
    parser.add_argument(
        "-lynxi-device", default=0, type=int, help="device id for lynxi"
    )
    ...
    output_path = lynxi_exchange.compile_lynxi_model(
        lynxi_model_path,
        net,
        in_data_type="float16",
    ...
    with torch.no_grad():
        for img, label in tqdm.tqdm(test_data_loader, disable=False):
            ...
            y = lynxi_exchange.lynxi_tensor_to_torch(
                y, shape=[label.shape[0], 10], dtype="float16"
            )
