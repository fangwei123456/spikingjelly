...
class SpikeActorDeepCritic(nn.Module):
    ...
    def act(self, obs, use_noise=True):
        with torch.no_grad():
            action = (
                self.san.act(obs).cpu().numpy()
                if not use_noise
                else self.san(obs).cpu().numpy()
            )
...
def hybrid_td3(
    ...
    def compute_loss_san(data):
        ...
        k = (
            K_FINAL
            * max(0, CUR_R - INF_R[env_name])
            / (SUP_R[env_name] - INF_R[env_name])
        )
    ...
    def get_action(o, use_noise=True):
        return ac.act(
            torch.as_tensor(o, dtype=torch.float32, device=device), use_noise=use_noise
        )
    ...
    def test_agent():
        ...
        for j in range(num_test_episodes):
            ...
            while not (d or (ep_len == max_ep_len)):
                ...
                o, r, d, _ = (
                    test_env.step(a[0]) if len(a.shape) > 1 else test_env.step(a)
                )
                ep_ret += r
                ep_len += 1
        ...
        if (t + 1) % steps_per_epoch == 0:
            ...
            if epoch % save_freq == 0:
                if test_mean_reward > max_test_reward:
                    ...
                    torch.save(
                        ac.san.state_dict(),
                        model_dir + "/model" + str(model_idx) + "_best.pt",
                    )
                    ac.san.to(device)
                    ...
                    pickle.dump(
                        ...
                        open(
                            model_dir + "/model" + str(model_idx) + "_best_mean_var.p",
                            "wb+",
                        ),
                    )
                    print(
            ...
            if epoch == epochs:
                ...
                torch.save(
                    ac.san.state_dict(),
                    model_dir + "/model" + str(model_idx) + "_last.pt",
                )
                ac.san.to(device)
                pickle.dump(
                    [replay_buffer.mean, replay_buffer.var],
                    open(
                        model_dir + "/model" + str(model_idx) + "_last_mean_var.p",
                        "wb+",
                    ),
                )
                print(
            ...
            noise_sigma = ac.san.get_noise_sigma()
            writer.add_scalar(
                tag="nsn", scalar_value=noise_sigma.item(), global_step=epoch
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="the exponent for the power-spectrum of the generated noise",
    )
    parser.add_argument(
        "--sigma_init",
        type=float,
        default=0.5,
        help="the sigma for the generated noise",
    )
