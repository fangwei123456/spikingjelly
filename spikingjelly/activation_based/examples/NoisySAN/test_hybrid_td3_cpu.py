...
def test_mujoco_render(
    ...
    def get_action(o):
        a = (
            san.act(torch.as_tensor(o, dtype=torch.float32, device=device))
            .cpu()
            .numpy()
        )
...
if __name__ == "__main__":
    ...
    for model_idx in range(args.num_model):
        ...
        test_reward, _ = pickle.load(
            open(file_dir + "/model" + str(model_idx) + "_test_rewards.p", "rb")
        )
        best_epoch_reward = 0
        for idx in range(20):
