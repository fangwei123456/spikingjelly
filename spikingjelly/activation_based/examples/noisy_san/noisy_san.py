...
class NoisySpikeMLP(nn.Module):
    def __init__(
        self, in_pop_dim, act_dim, dec_pop_dim, hidden_sizes, spike_ts, beta, sigma_init
    ):
        super().__init__()
...
class NoisyPopSpikeDecoder(nn.Module):
    def __init__(self, act_dim, pop_dim, spike_ts, beta, sigma_init):
        ...
        self.decoder = neuron.NoisyNonSpikingIFNode(
            act_dim, T=spike_ts, sigma_init=sigma_init, beta=beta, decode="last-mem"
        )

        functional.set_step_mode(self, step_mode="m")

    def forward(self, out_pop_spikes):
        out_pop_spikes = out_pop_spikes.view(
            self.spike_ts, -1, self.act_dim, self.pop_dim
        )
        return self.decoder(
            self.group_fc(out_pop_spikes).view(self.spike_ts, -1, self.act_dim)
        )
...
class NoisyPopSpikeActor(nn.Module):
    def __init__(
        ...
        self.encoder = encoding.PopSpikeEncoderDeterministic(
            obs_dim, enc_pop_dim, spike_ts, mean_range, std
        )
        self.snn = NoisySpikeMLP(
            obs_dim * enc_pop_dim,
            act_dim,
            dec_pop_dim,
            hidden_sizes,
        ...
        self.decoder = NoisyPopSpikeDecoder(
            act_dim, dec_pop_dim, spike_ts, beta, sigma_init
        )
    ...
    def get_colored_noise_length(self):
        return (
            self.snn.get_colored_noise_length()
            + self.decoder.get_colored_noise_length()
        )
