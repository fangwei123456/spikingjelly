...
def make_test_env(params):
    ...
    env = atari_wrappers.wrap_deepmind(
        env, episode_life=False, clip_rewards=False, frame_stack=True, pytorch_img=True
    )
    return env
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="max-mem",
        help="The type of SNN decoder, e.g. max-mem, mean-mem, max-abs-mem, last-mem, fr-mlp",
    )
    parser.add_argument(
        "--early_stop",
        default=False,
        action="store_true",
        help="Use stop reward to stop early",
    )
    parser.add_argument(
        "--eval_q", default=False, action="store_true", help="Record the Q-value (eval)"
    )
    parser.add_argument(
        "--sticky_actions",
        default=False,
    ...
    parser.add_argument(
        "--frame_num", type=int, default=20 * 10**6, help="The number of frames"
    )
        ...
        while True:
            ...
            if frame_idx % MODEL_SAVE_FREQUENCY == 0:
                ...
                if test_reward > max_reward:
                    ...
                    torch.save(
                        net.state_dict(),
                        os.path.join(results_dir, "dsqn_" + args.game + ".pth"),
                    )

                print("------------------------------------------------")
                test_writer.add_scalar(game + "/reward", test_reward, frame_idx)
                print(
                    "F: %d, Max-F: %d, R: %.1f, Max-R: %.1f"
                    % (frame_idx, max_frame_idx, test_reward, max_reward)
                )
                ...
                backup_path = os.path.join(
                    results_dir, "dsqn_" + args.game + "_backup.pth"
                )
