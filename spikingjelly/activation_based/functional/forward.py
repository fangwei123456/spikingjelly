...
def seq_to_ann_forward(
    ...
    * **中文**
    ...
    :raises ValueError: 当 ``x_seq`` 为空tuple，或tuple中tensor的维数少于2，或tuple中tensor的 ``[T, batch_size]`` 前两维不一致时抛出
    ...
    * **English**
    ...
    :raises ValueError: if ``x_seq`` is an empty tuple, if any tensor in ``x_seq`` has fewer than 2 dimensions, or if the tensors in ``x_seq`` do not share the same ``[T, batch_size]`` leading dimensions
