...
@triton.jit
def _single_step_activation_aware_if_forward(
    x_ptr,
    v_init_ptr,
    threshold_ptr,
    offset_ptr,
...
def _launch_single_step_activation_aware_if_forward(
    x: torch.Tensor,
    v_init: torch.Tensor,
    threshold: torch.Tensor,
    offset: torch.Tensor,
    ...
    *,
    ...
    with torch.cuda.device(x.device):
        _single_step_activation_aware_if_forward[grid](
            x,
            v_init,
            threshold,
            offset,
...
@register_op("sj::single_step_activation_aware_if_inference")
def _single_step_activation_aware_if_inference(
    x: torch.Tensor,
    v_init: torch.Tensor,
    threshold: torch.Tensor,
    offset: torch.Tensor,
    ...
    threshold = threshold.to(device=x.device, dtype=x.dtype).contiguous()
    offset = offset.to(device=x.device, dtype=x.dtype).contiguous()
    spike = torch.empty_like(x)
    v = torch.empty_like(v_init)
    _launch_single_step_activation_aware_if_forward(
        x,
        v_init,
        threshold,
        offset,
...
@torch.library.register_fake("sj::single_step_activation_aware_if_inference")
def _single_step_activation_aware_if_inference_fake(
    x: torch.Tensor,
    v_init: torch.Tensor,
    threshold: torch.Tensor,
    offset: torch.Tensor,
    ...
    return torch.empty_like(x), torch.empty_like(v_init)


def _single_step_activation_aware_if(
    x: torch.Tensor,
    v_init: torch.Tensor,
    threshold: torch.Tensor,
    offset: torch.Tensor,
    *,
    ...
    return _single_step_activation_aware_if_inference(
        x,
        v_init,
        threshold,
        offset,
