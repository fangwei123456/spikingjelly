...
__all__ = ["single_step_if", "multistep_if"]


@triton.jit
def _single_step_if_forward_kernel(
    x_ptr,
    v_init_ptr,
    spike_ptr,
    h_ptr,
...
@triton.jit
def _single_step_if_backward_kernel(
    grad_spike_ptr,
    grad_v_ptr,
    h_ptr,
    grad_x_ptr,
    ...
    tl.store(grad_v_init_ptr + offsets, grad_h, mask=mask)
...
def _launch_single_step_if_forward(
    x: torch.Tensor,
    v_init: torch.Tensor,
    spike: torch.Tensor,
    h: torch.Tensor,
    ...
    *,
    ...
    with torch.cuda.device(x.device):
        wrap_triton(_single_step_if_forward_kernel)[grid](
            x,
            v_init,
            spike,
            h,
...
def _launch_single_step_if_backward(
    ...
    *,
    ...
    with torch.cuda.device(grad_spike.device):
        wrap_triton(_single_step_if_backward_kernel)[grid](
            grad_spike,
            grad_v,
            h,
            grad_x,
...
@register_op("sj::single_step_if_inference")
def single_step_if_inference(
    x: torch.Tensor,
    v_init: torch.Tensor,
    v_threshold: float,
    v_reset: float,
    ...
    _launch_single_step_if_forward(
        x,
        v_init,
        spike,
        v,
...
@torch.library.register_fake("sj::single_step_if_inference")
def _single_step_if_inference_fake(
    x: torch.Tensor,
    v_init: torch.Tensor,
    v_threshold: float,
    v_reset: float,
...
@register_op("sj::single_step_if_forward")
def single_step_if_forward(
    x: torch.Tensor,
    v_init: torch.Tensor,
    v_threshold: float,
    v_reset: float,
...
@torch.library.register_fake("sj::single_step_if_forward")
def _single_step_if_forward_fake(
    x: torch.Tensor,
    v_init: torch.Tensor,
    v_threshold: float,
    v_reset: float,
...
def _single_step_if_backward(ctx, grad_spike, grad_v, grad_h):
    (h,) = ctx.saved_tensors
    ...
    _launch_single_step_if_backward(
        grad_spike,
        grad_v,
        h,
        grad_x,
...
def single_step_if(
    ...
    r"""
    **API Language** - :ref:`中文 <single_step_if-cn>` | :ref:`English <single_step_if-en>`
    ...
    * **中文**
    ...
    使用专用 Triton kernel 执行单步 IF 状态转移。``x`` 和 ``v_init`` 必须是
    ...
    * **English**
    ...
    Run one IF state transition with the dedicated Triton kernel. ``x`` and
    ...
    if x.device.type != "cuda":
        raise ValueError("single_step_if requires CUDA tensors.")
    ...
    if need_grad:
        sg_triton_id, sg_alpha = resolve_sg_triton_id_and_alpha(surrogate_function)
        spike, v, _ = single_step_if_forward(
            x,
            v_init,
            v_threshold,
            reset,
        ...
        return spike, v
    return single_step_if_inference(x, v_init, v_threshold, reset, soft_reset)
