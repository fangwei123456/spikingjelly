...
__all__ = ["single_step_plif", "multistep_plif"]


@triton.jit
def _single_step_plif_forward_kernel(
    x_ptr,
    v_init_ptr,
    r_tau_ptr,
    spike_ptr,
...
@triton.jit
def _single_step_plif_backward_kernel(
    grad_spike_ptr,
    grad_v_ptr,
    h_ptr,
    v_init_ptr,
    ...
    if decay_input:
        grad_x = grad_h * r_tau
        grad_r_tau = grad_h * (h - v_init) / r_tau
    else:
        grad_x = grad_h
        grad_r_tau = grad_h * (reset - v_init)
    grad_v_init = grad_h * (1.0 - r_tau)

    tl.store(grad_x_ptr + offsets, grad_x, mask=mask)
    tl.store(grad_v_init_ptr + offsets, grad_v_init, mask=mask)
    tl.store(grad_r_tau_ptr + offsets, grad_r_tau, mask=mask)
...
def _launch_single_step_plif_forward(
    x: torch.Tensor,
    v_init: torch.Tensor,
    r_tau: torch.Tensor,
    spike: torch.Tensor,
    ...
    *,
    ...
    with torch.cuda.device(x.device):
        wrap_triton(_single_step_plif_forward_kernel)[grid](
            x,
            v_init,
            r_tau,
            spike,
...
def _launch_single_step_plif_backward(
    ...
    *,
    ...
    with torch.cuda.device(grad_spike.device):
        wrap_triton(_single_step_plif_backward_kernel)[grid](
            grad_spike,
            grad_v,
            h,
            v_init,
...
@register_op("sj::single_step_plif_inference")
def single_step_plif_inference(
    x: torch.Tensor,
    v_init: torch.Tensor,
    r_tau: torch.Tensor,
    decay_input: bool,
    ...
    x = x.contiguous()
    v_init = v_init.contiguous()
    r_tau = r_tau.contiguous()
    spike = torch.empty_like(x)
    v = torch.empty_like(v_init)
    _launch_single_step_plif_forward(
        x,
        v_init,
        r_tau,
        spike,
...
@torch.library.register_fake("sj::single_step_plif_inference")
def _single_step_plif_inference_fake(
    x: torch.Tensor,
    v_init: torch.Tensor,
    r_tau: torch.Tensor,
    decay_input: bool,
...
@register_op("sj::single_step_plif_forward")
def single_step_plif_forward(
    x: torch.Tensor,
    v_init: torch.Tensor,
    r_tau: torch.Tensor,
    decay_input: bool,
...
@torch.library.register_fake("sj::single_step_plif_forward")
def _single_step_plif_forward_fake(
    x: torch.Tensor,
    v_init: torch.Tensor,
    r_tau: torch.Tensor,
    decay_input: bool,
...
def _setup_single_step_context(ctx, inputs, output):
    ...
    ctx.save_for_backward(output[2], inputs[1], inputs[2])
    ctx.decay_input = decay_input
    ctx.v_threshold = v_threshold
    ctx.v_reset = v_reset
    ctx.soft_reset = soft_reset
...
def _single_step_plif_backward(ctx, grad_spike, grad_v, grad_h):
    h, v_init, r_tau = ctx.saved_tensors
    ...
    grad_r_tau_elements = torch.empty_like(grad_spike)
    _launch_single_step_plif_backward(
        grad_spike,
        grad_v,
        h,
        v_init,
    ...
    grad_r_tau = grad_r_tau_elements.sum().reshape_as(r_tau)
    return (
        grad_x,
        grad_v_init,
        grad_r_tau,
...
torch.library.register_autograd(
    "sj::single_step_plif_forward",
    _single_step_plif_backward,
    setup_context=_setup_single_step_context,
)


def single_step_plif(
    x: torch.Tensor,
    v_init: torch.Tensor,
    r_tau: torch.Tensor,
    decay_input: bool,
    ...
    r"""
    **API Language** - :ref:`中文 <single_step_plif-cn>` | :ref:`English <single_step_plif-en>`

    ----

    .. _single_step_plif-cn:

    * **中文**

    使用专用 Triton kernel 执行单步 Parametric LIF 状态转移。``x`` 和
    ``v_init`` 必须是 shape、dtype 和 device 相同的 CUDA FP32、FP16 或
    BF16 张量；``r_tau`` 必须是同 device/dtype 的单元素张量。

    :param x: 单步 CUDA 输入，形状为 ``[N, *]``
    :type x: torch.Tensor
    :param v_init: 与 ``x`` 同形状的初始膜电位
    :type v_init: torch.Tensor
    :param r_tau: 可学习膜时间常数的倒数，必须为单元素张量
    ...
    :raises ValueError: 当 ``x`` 和 ``v_init`` 的 shape、dtype 或 CUDA device
        不一致，输入不在 CUDA 上，或 ``r_tau`` 不是与输入同 device/dtype 的
        单元素张量时
    :raises NotImplementedError: 当 dtype 或替代函数不受 Triton 后端支持时

    ----

    .. _single_step_plif-en:

    * **English**

    Run one Parametric LIF state transition with the dedicated Triton kernel.
    ``x`` and ``v_init`` must be CUDA FP32, FP16, or BF16 tensors with matching
    shape, dtype, and device. ``r_tau`` must be a scalar tensor on the same
    device and with the same dtype.

    :param x: Single-step CUDA input shaped ``[N, *]``
    :type x: torch.Tensor
    :param v_init: Initial membrane voltage with the same shape as ``x``
    :type v_init: torch.Tensor
    :param r_tau: Reciprocal of the learnable membrane time constant; a scalar tensor
    :type r_tau: torch.Tensor
    ...
    :raises ValueError: If ``x`` and ``v_init`` differ in shape, dtype, or CUDA
        device, if the input is not on CUDA, or if ``r_tau`` is not a scalar
        tensor with the same device and dtype as the input
    :raises NotImplementedError: If the dtype or surrogate function is not
        supported by the Triton backend
    """
    if x.device.type != "cuda":
        raise ValueError("single_step_plif requires CUDA tensors.")
    ...
    if r_tau.numel() != 1 or r_tau.dtype != x.dtype or r_tau.device != x.device:
        raise ValueError(
            "r_tau must be a scalar tensor with the same dtype and device as x."
        )
    ...
    need_grad = torch.is_grad_enabled() and (
        x.requires_grad or v_init.requires_grad or r_tau.requires_grad
    )
    if need_grad:
        sg_triton_id, sg_alpha = resolve_sg_triton_id_and_alpha(surrogate_function)
        spike, v, _ = single_step_plif_forward(
            x,
            v_init,
            r_tau,
            decay_input,
        ...
        return spike, v
    return single_step_plif_inference(
        x,
        v_init,
        r_tau,
        decay_input,
