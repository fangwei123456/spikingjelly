...
try:
    ...
    logger.debug("FP8 capability dependency unavailable: {}", e)
    triton = None
    tl = None
...
if triton is not None:
    ...
    def _fp8_neuron_backward_probe_kernel(
        ...
        for t in tl.static_range(1, -1, -1):
            ...
            s = tl.where(h >= 1.0, 1.0, 0.0).to(compute_dtype)
            over_th = (alpha * (h - 1.0)).to(tl.float32)
...
def triton_fp8_neuron_capability_report(
    ...
    **API Language** - 中文 | English
    ...
    logger.info(
        "FP8 capability checked: device={} triton_available={} cuda_available={} compute_dtypes={}",
        device,
