...
def flexsn_inference(f, info: FlexSNInfo, *args) -> tuple:
    ...
    outputs = [
        torch.empty_like(x_example)
        for _ in range(info.num_outputs+info.num_states)
    ]
...
def flexsn_forward(f, info: FlexSNInfo, *args) -> tuple:
    ...
    returns = [
        torch.empty_like(x_example)
        for _ in range(info.num_fwd_kernel_returns)
    ]
    dtype = x_example.dtype
    grid = lambda meta: (triton.cdiv(NCL, meta['BLOCK_NCL']),)
...
def flexsn_backward(f, info: FlexSNInfo, *args) -> tuple:
    ...
    grad_inputs = [
        torch.empty_like(grad_example) for _ in range(info.num_inputs)
    ]
    grad_inputs += [
        torch.empty_like(grad_example[0]) for _ in range(info.num_states)
    ]
...
class FlexSNFunction(autograd.Function):
    ...
    def backward(ctx, *args):  # len(args) = num_outputs + num_states
        ...
        grads = flexsn_backward(
            fn_bwd, ctx.info, *args, *required_results
        )
