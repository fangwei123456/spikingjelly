...
class MultiStepIFNodePTT(torch.autograd.Function):
    ...
    def backward(ctx, grad_spike_seq, grad_v_seq):
        ...
        kernel = MultiStepIFNodePTT.create_bptt_kernel(
...
class MultiStepLIFNodePTT(torch.autograd.Function):
    ...
    def forward(
        ...
        with cuda_utils.DeviceEnvironment(device):
            ...
            kernel = MultiStepLIFNodePTT.create_fptt_kernel(
    ...
    def backward(ctx, grad_spike_seq, grad_v_seq):
        ...
        kernel = MultiStepLIFNodePTT.create_bptt_kernel(
...
class MultiStepParametricLIFNodePTT(torch.autograd.Function):
    ...
    def create_fptt_kernel(decay_input: bool, hard_reset: bool, dtype: str):
        return MultiStepLIFNodePTT.create_fptt_kernel(
    ...
    def forward(
        ...
        with cuda_utils.DeviceEnvironment(device):
            ...
            kernel = MultiStepParametricLIFNodePTT.create_fptt_kernel(
                decay_input, hard_reset, dtype
            )
    ...
    def backward(ctx, grad_spike_seq, grad_v_seq):
        ...
        kernel = MultiStepParametricLIFNodePTT.create_bptt_kernel(
            ctx.sg_cuda_code_fun, ctx.decay_input, hard_reset, ctx.detach_reset, dtype
        )
...
def check_multi_step_neuron_output_and_grad(
    device, multi_step_neuron, shape=[65, 15, 511], *neu_args, **neu_kwargs
):
    @torch.no_grad()
    def max_error(x, y):
    ...
    for hard_reset in [True, False]:
        for detach_reset in [False, True]:
            for dtype in ["fp32", "fp16"]:
                x = (torch.rand(shape, device=device) - 0.5) * 3.0
                if dtype == "fp16":
                    x = x.half()
                print(
                    f"hard_reset={hard_reset}, detach_reset={detach_reset}, dtype={dtype}"
                )
                model = multi_step_neuron(
                    v_reset=0.0 if hard_reset else None,
                ...
                for key in y_torch.keys():
                    ...
                    if me > 0.5:
                        print(
                            f"y_torch[{key}]={y_torch[key]}, y_cupy[{key}]={y_cupy[key]}"
                        )
                print("\n")


def check_single_step_neuron_output_and_grad(
    device, single_step_neuron, shape=[65, 15, 511], *neu_args, **neu_kwargs
):
    @torch.no_grad()
    def max_error(x, y):
        return (x - y).abs().max().item()
...
class MultiStepQIFNodePTT(torch.autograd.Function):
    ...
    def backward(ctx, grad_spike_seq, grad_v_seq):
        ...
        kernel = MultiStepQIFNodePTT.create_bptt_kernel(
            ctx.sg_cuda_code_fun, hard_reset, ctx.detach_reset, dtype
        )
...
class MultiStepIzhikevichNodePTT(torch.autograd.Function):
    ...
    def backward(ctx, grad_spike_seq, grad_v_seq, grad_w_seq):
        ...
        kernel = MultiStepIzhikevichNodePTT.create_bptt_kernel(
            ctx.sg_cuda_code_fun, hard_reset, ctx.detach_reset, dtype
        )
...
class MultiStepEIFNodePTT(torch.autograd.Function):
    ...
    def backward(ctx, grad_spike_seq, grad_v_seq):
        ...
        kernel = MultiStepEIFNodePTT.create_bptt_kernel(
            ctx.sg_cuda_code_fun, hard_reset, ctx.detach_reset, dtype
        )
                ...
                for hard_reset in [True, False]:
                    for dtype in ["fp32", "fp16"]:
                        ...
                        for detach_reset in [True, False]:
                            ...
                            bp_codes = ms_neu.create_bptt_kernel(
                                sg().cuda_code, hard_reset, detach_reset, dtype
                            ).code
