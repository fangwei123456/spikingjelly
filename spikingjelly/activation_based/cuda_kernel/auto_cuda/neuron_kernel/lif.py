...
def cupy_multistep_lif_backward(
...
@torch.library.register_fake("sj::cupy_multistep_lif_backward")
def _cupy_multistep_lif_backward_fake(
    grad_spike_seq: torch.Tensor,
    grad_v_seq: torch.Tensor,
    h_seq: torch.Tensor,
    v_th: float,
...
def _cupy_multistep_lif_backward_autograd(ctx, grad_spike_seq, grad_v_seq, grad_h_seq):
    del grad_h_seq
    (h_seq,) = ctx.saved_tensors
    soft_reset = ctx.v_reset is None
    v_reset = 0.0 if soft_reset else float(ctx.v_reset)
    grad_x_seq, grad_v_init = cupy_multistep_lif_backward(
        grad_spike_seq,
        grad_v_seq,
        h_seq,
        ctx.v_th,
