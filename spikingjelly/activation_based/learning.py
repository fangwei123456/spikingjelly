...
def mstdp_linear_single_step(
    ...
    eligibility = f_post(weight, w_max) * (trace_pre.unsqueeze(1) * out_spike.unsqueeze(2)) -\
                  f_pre(weight, w_min) * (trace_post.unsqueeze(2) * in_spike.unsqueeze(1))
...
def mstdpet_linear_single_step(
    ...
    eligibility = f_post(weight, w_max) * torch.outer(out_spike, trace_pre) -\
                  f_pre(weight, w_min) * torch.outer(trace_post, in_spike)
    return trace_pre, trace_post, eligibility
...
def stdp_multi_step(
    ...
    for t in range(T):
        trace_pre, trace_post, dw = stdp_single_step(
            ...
            tau_pre, tau_post, w_min, w_max, f_pre, f_post
        )
        delta_w += dw
...
class MSTDPETLearner(base.MemoryModule):
    def __init__(
        ...
        tau_pre: float, tau_post: float, tau_trace: float, w_min: float, w_max: float,
        f_pre: Callable = lambda x: x, f_post: Callable = lambda x: x
    ):
        super().__init__()
        self.step_mode = step_mode
    ...
    def step(self, reward, on_grad: bool = True, scale: float = 1.):
        ...
        for _ in range(length):
            ...
            self.trace_pre, self.trace_post, self.eligibility = stdp_f(
                ...
                self.w_min, self.w_max, self.f_pre, self.f_post
            )
