...
class Tempotron(nn.Module):
    def __init__(
        self, in_features, out_features, T, tau=15.0, tau_s=15.0 / 4, v_threshold=1.0
    ):
        """
    ...
    def mse_loss(v_max, v_threshold, label, num_classes):
        ...
        """
        wrong_mask = (
            (v_max >= v_threshold).float() != F.one_hot(label, num_classes)
        ).float()
        return (
            torch.sum(torch.pow((v_max - v_threshold) * wrong_mask, 2)) / label.shape[0]
        )
    ...
    def forward(self, in_spikes: torch.Tensor, ret_type):
        ...
        表示无脉冲发放
        """
        t = torch.arange(0, self.T).to(
            in_spikes.device
        )  # t = [0, 1, 2, ..., T-1] shape=[T]
        t = t.view(1, 1, t.shape[0]).repeat(
            in_spikes.shape[0], in_spikes.shape[1], 1
        )  # shape=[batch_size, in_features, T]
        in_spikes = in_spikes.unsqueeze(-1).repeat(
            1, 1, self.T
        )  # shape=[batch_size, in_features, T]
        v_in = (
            self.v0
            * self.psp_kernel(t - in_spikes, self.tau, self.tau_s)
            * (in_spikes >= 0).float()
        )  # in_spikes[:, i] < 0的位置，输入电压也为0
        v_out = self.fc(v_in.permute(0, 2, 1)).permute(
            0, 2, 1
        )  # shape=[batch_size, out_features, T]
        ...
        if ret_type == "v":
            ...
            t = t.view(1, 1, t.shape[0]).repeat(
                in_spikes.shape[0], v_out.shape[1], 1
            )  # shape=[batch_size, out_features, T]
            max_index_soft = (F.softmax(v_out * self.T, dim=2) * t).sum(
                dim=2
