...
class GaussianTuning:
    def __init__(self, n, m, x_min: torch.Tensor, x_max: torch.Tensor):
        ...
        self.n = n
        i = (
            torch.arange(1, m + 1).unsqueeze(0).repeat(n, 1).float().to(x_min.device)
        )  # shape=[n, m]
    ...
    def encode(self, x: torch.Tensor, max_spike_time=50):
        ...
        # 计算x对应的m个高斯函数值
        y = torch.exp(
            -(x - self.mu).square() / 2 / self.sigma2
        )  # shape=[batch_size * k, n, m]
        ...
        out_spikes = out_spikes.view(
            x_shape[0], x_shape[2], out_spikes.shape[1], out_spikes.shape[2]
        )
        out_spikes = out_spikes.permute(
            0, 2, 1, 3
        )  # shape: [batch_size, k, n, m] -> [batch_size, n, k, m]
        return out_spikes
