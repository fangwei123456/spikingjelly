...
def main() -> None:
    ...
    if min(prompt_counts) <= 0 or len(tokens) == 0:
        raise ValueError("prompt counts and the dataset must be non-empty.")
    ...
    try:
        ...
        for prompt_count in prompt_counts:
            ...
            for _ in range(args.warmups):
                engine.generate(input_ids=prompts.tolist(), sampling_params=sampling)
            elapsed_samples = []
            peak_memory_bytes = 0
            results = None
            for _ in range(args.repeats):
                ...
                results = engine.generate(
                    input_ids=prompts.tolist(), sampling_params=sampling
                )
                elapsed_samples.append(time.perf_counter() - started)
                peak_memory_bytes = max(peak_memory_bytes, _peak_device_memory_bytes())
