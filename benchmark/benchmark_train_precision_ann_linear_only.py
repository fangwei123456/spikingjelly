...
class LinearStack(torch_nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_classes: int, depth: int):
        super().__init__()
        if depth < 2:
        ...
        for _ in range(depth - 1):
            layers.append(torch_nn.Linear(in_dim, hidden_dim))
            in_dim = hidden_dim
        layers.append(torch_nn.Linear(hidden_dim, num_classes))
...
def build_model(args: argparse.Namespace) -> LinearStack:
    return LinearStack(
        input_dim=args.input_dim,
        hidden_dim=args.hidden_dim,
        num_classes=args.num_classes,
