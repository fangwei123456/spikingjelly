...
Compared variants:
  - backend="torch"
  - backend="hop" + torch.compile(fullgraph=True)
  - backend="triton"
...
def bench_single_layer():
    ...
    print(
        f"  {'T':>4} {'B':>5} {'N':>6}  "
        f"{'torch':>10}  {'hop+cmp':>10}  {'triton':>10}  {'tri+cmp':>10}"
    )
    print("-" * 92)
    ...
    for T, B, N in configs:
        ...
        n_hop = make_flexsn("hop")
        n_triton = make_flexsn("triton")
        n_triton_compiled = make_flexsn("triton")
        compiled_hop = torch.compile(n_hop, fullgraph=True)
        compiled_triton = torch.compile(n_triton_compiled, fullgraph=True)
        ...
        with torch.no_grad():
            ...
            ms_hop = cuda_time_ms(
                lambda c=compiled_hop, x=x: c(x),
                reset_hook=(lambda r=n_hop.reset: r()),
            )
            ms_triton_compiled = cuda_time_ms(
                lambda c=compiled_triton, x=x: c(x),
...
def bench_linear_flexsn_linear():
    ...
    for T, B, N in configs:
        ...
        m_hop = SeqModelFused(N, "hop").cuda()
        m_triton = SeqModelFused(N, "triton").cuda()
        m_triton_compiled = SeqModelFused(N, "triton").cuda()
        compiled_hop = torch.compile(m_hop, fullgraph=True)
        compiled_triton = torch.compile(m_triton_compiled, fullgraph=True)
        ...
        with torch.no_grad():
            ...
            ms_hop = cuda_time_ms(
                lambda c=compiled_hop, x=x: c(x),
                reset_hook=(lambda m=m_hop: functional.reset_net(m)),
            )
            ms_triton_compiled = cuda_time_ms(
                lambda c=compiled_triton, x=x: c(x),
...
def main() -> None:
    ...
    print()
    print("Columns: torch eager | HOP compiled | Triton eager | Triton compiled (ms)")
    print()
