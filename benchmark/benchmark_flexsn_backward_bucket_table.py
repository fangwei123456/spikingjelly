...
def build_workloads():
    return [
        {
            "name": "cifar_vgg11_bn",
            "builder": lambda: spiking_vgg11_bn(
                pretrained=False,
        ...
        },
        {
            ...
            "builder": lambda: spiking_vgg16_bn(
                pretrained=False,
                spiking_neuron=make_flexsn,
                num_classes=1000,
                step_mode="m",
...
def main():
    ...
    for workload in build_workloads():
        ...
        print(f"\n## {workload['name']}")
        print(
            "| step_shape | NCL | bucket | layers | bench_mean_ms | bench_median_ms |"
        )
        print("| --- | ---: | --- | ---: | ---: | ---: |")
        for (shape, ncl, bucket), layers in sorted(
    ...
    for bucket, entries in sorted(summary.items()):
        ...
        print(
            f"| {BUCKET_LABELS[bucket]} | {len(entries)} | {max(ncls)} | {min(ncls)} |"
        )
