...
-----------------------
Now let us use the CUDA-Enhanced Multi-Step neuron to re-implement the network in :doc:`./4_conv_fashion_mnist` and compare their speeds. There is no need to modify the training codes. We can only change the network's codes:
...
The fully codes are available at :class:`spikingjelly.activation_based.examples.conv_fashion_mnist`. Run this example with the same arguments and devices as those in :doc:`./4_conv_fashion_mnist`. The outputs are:
