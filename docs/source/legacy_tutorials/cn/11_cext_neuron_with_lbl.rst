...
加速深度脉冲神经网络
-----------------------
现在让我们使用多步和cupy后端神经元，重新实现 :doc:`./4_conv_fashion_mnist` 中的网络。我们只需要更改一下网络结构，无需进行
其他的改动：
...
:doc:`./4_conv_fashion_mnist` 中完全相同的输入参数和设备（`Intel(R) Xeon(R) Gold 6148 CPU @ 2.40GHz` 的CPU
