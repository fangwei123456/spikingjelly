时间驱动：编码器
=======================================
本教程作者： `Grasshlw <https://github.com/Grasshlw>`_, `Yanqi-Chen <https://github.com/Yanqi-Chen>`_
...
对于有 ``M`` 个特征的输入数据，高斯调谐曲线编码器使用 ``tuning_curve_num`` 个神经元去编码输入数据的每一个特征，将每个特征编码为这 ``tuning_curve_num`` 个
