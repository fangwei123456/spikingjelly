...
def plot_log(csv_file, title, x_label, y_label, figsize=(12, 8), plot_max=False):
    ...
    if plot_max:
        ...
        plt.text(
            x[index],
            y[index],
            "({}, {})".format(int(x[index]), round(y[index], 3)),
            fontsize=14,
        )
...
if __name__ == "__main__":
    ...
    plot_log(
        "./docs/source/_static/tutorials/4_conv_fashion_mnist/run-logs-tag-train_accuracy.csv",
        "Accuracy on train batch",
        "iteration",
        "accuracy",
    )
    plt.savefig("./docs/source/_static/tutorials/4_conv_fashion_mnist/train.svg")
    plt.savefig("./docs/source/_static/tutorials/4_conv_fashion_mnist/train.pdf")
    plt.savefig("./docs/source/_static/tutorials/4_conv_fashion_mnist/train.png")
    plt.clf()
    plot_log(
        "./docs/source/_static/tutorials/4_conv_fashion_mnist/run-logs-tag-test_accuracy.csv",
        "Accuracy on test dataset",
        "epoch",
        "accuracy",
        plot_max=True,
    )
    plt.savefig("./docs/source/_static/tutorials/4_conv_fashion_mnist/test.svg")
    plt.savefig("./docs/source/_static/tutorials/4_conv_fashion_mnist/test.pdf")
    plt.savefig("./docs/source/_static/tutorials/4_conv_fashion_mnist/test.png")
    exit()
    dataset_dir = input(
    ...
    encoder = nn.Sequential(net.static_conv, net.conv[0])
    encoder.eval()
        ...
        for img, label in test_data_loader:
            ...
            for t in range(net.T):
                ...
                if t == 0 or t == net.T - 1:
                    ...
                    visualizing.plot_2d_spiking_feature_map(
